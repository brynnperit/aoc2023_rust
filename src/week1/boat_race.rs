...
fn get_count_of_winning_strategies_from_input(input: clio::Input) -> Vec<u64> {
    ...
    while let Ok(race_time) = time_line_iter.next().unwrap_or("").parse() {
    ...
    while let Ok(race_distance) = distance_line_iter.next().unwrap_or("").parse() {
        race_distances.push(race_distance);
    }

    let mut boat_races = Vec::new();
    loop {
        match race_times.pop() {
            Some(race_time) => match race_distances.pop() {
                Some(race_distance) => boat_races.push(BoatRace::new(race_time, race_distance)),
                None => break,
            },
            None => break,
        }
...
fn get_count_of_winning_strategies_from_input_as_single_race(input: clio::Input) -> u64 {
    ...
    let mut time_line_iter = time_line.split_ascii_whitespace();
    time_line_iter.next();
    let race_time = time_line_iter.fold("".to_string(), |a, b| a + b).parse().unwrap();
    ...
    get_count_of_winning_strategies_for_boat_races(vec![BoatRace::new(race_time, race_distance)]).pop().unwrap_or_default()
}
...
fn get_count_of_winning_strategies_for_boat_races(boat_races: Vec<BoatRace>) -> Vec<u64>{
    ...
    for boat_race in boat_races{
        ...
        let lower_bound = (-quad_b-(quad_b*quad_b-4_f64*quad_a*quad_c).sqrt())/(2_f64*quad_a);
        let upper_bound = (-quad_b+(quad_b*quad_b-4_f64*quad_a*quad_c).sqrt())/(2_f64*quad_a);
