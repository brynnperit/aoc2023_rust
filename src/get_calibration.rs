...
static DIGIT_REGEX_INCLUDE_WORDS: Lazy<Regex> = Lazy::new(|| Regex::new(r"([0-9]|one|two|three|four|five|six|seven|eight|nine)").unwrap());

pub fn get_calibration_value(line: &str, use_words_for_digits: bool) -> i32 {
    let digit_regex = if use_words_for_digits {&DIGIT_REGEX_INCLUDE_WORDS} else {&DIGIT_REGEX};
...
static DIGIT_WORDS: Lazy<HashMap<&str, i32>> = Lazy::new(|| HashMap::from([("one",1),("two",2),("three",3),("four",4),("five",5),("six",6),("seven",7),("eight",8),("nine",9)]));
